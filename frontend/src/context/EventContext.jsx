...
export const EventProvider = ({ children }) => {
  ...
  const getEvents = async (searchParams = {}) => {
    try {
      setLoading(true);
      const queryString = new URLSearchParams(searchParams).toString();
      const res = await axios.get(`/events?${queryString}`);  // fixed here
      const normalized = res.data.data.map(normalizeEvent);
      dispatch({ type: 'SET_EVENTS', payload: normalized });
    } catch (error) {
      setError(error.response?.data?.message || 'Failed to fetch events');
  ...
  const getEventById = async (id) => {
    try {
      setLoading(true);
      const res = await axios.get(`/events/${id}`); // fixed here
      const evt = normalizeEvent(res.data.data);
      dispatch({ type: 'SET_CURRENT_EVENT', payload: evt });
      return res.data;
    } catch (error) {
  ...
  const createEvent = async (formData) => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token'); // Get JWT token
      const res = await axios.post('/events', formData, {  // fixed here
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const updateEvent = async (id, formData) => {
    try {
      ...
      const res = await axios.put(`/events/${id}`, formData);  // fixed here
      dispatch({ type: 'UPDATE_EVENT', payload: normalizeEvent(res.data.data) });
      toast.success('Event updated successfully!');
      return res.data;
    } catch (error) {
  ...
  const deleteEvent = async (id) => {
    try {
      setLoading(true);
      await axios.delete(`/events/${id}`);  // fixed here
      dispatch({ type: 'DELETE_EVENT', payload: id });
      toast.success('Event deleted successfully!');
    } catch (error) {
      const message = error.response?.data?.message || 'Failed to delete event';
  ...
  const toggleRSVP = async (id) => {
    try {
      const res = await axios.post(`/events/${id}/rsvp`);  // fixed here
      dispatch({ type: 'TOGGLE_RSVP', payload: normalizeEvent(res.data.data) });
      toast.success(res.data.message || 'RSVP updated successfully!');
      return res.data;
    } catch (error) {
  ...
  const getUserEvents = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/events/user/registered');  // fixed here
      const normalized = res.data.data.map(normalizeEvent);
      dispatch({ type: 'SET_USER_EVENTS', payload: normalized });
    } catch (error) {
      setError(error.response?.data?.message || 'Failed to fetch user events');
  ...
  const getAdminEvents = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/events/admin/my-events');  // fixed here
      const normalized = res.data.data.map(normalizeEvent);
      dispatch({ type: 'SET_USER_EVENTS', payload: normalized });
    } catch (error) {
      setError(error.response?.data?.message || 'Failed to fetch admin events');
